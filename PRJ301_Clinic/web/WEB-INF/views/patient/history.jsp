    ...
    <div class="glass-card p-4 mb-4 animate-fade-in">
        <div class="row align-items-center g-3">
            <div class="col-md-8 d-flex align-items-center gap-3">
                <div class="avatar-circle bg-primary-subtle text-info fs-2 fw-bold border border-info border-2" style="width: 65px; height: 65px; display: flex; align-items: center; justify-content: center; border-radius: 50%;">
                ...
                <div>
                    ...
                    <p class="text-info mb-0 small">
                        <i class="fa-solid fa-user-check me-1"></i>Bệnh nhân: ${sessionScope.LOGIN_USER.fullname} | 
                        <i class="fa-solid fa-receipt ms-2 me-1"></i>Tổng cộng: ${totalItems} cuộc hẹn
                    </p>
                </div>
            </div>

            <div class="col-md-4 text-md-end">
                <a href="${pageContext.request.contextPath}/MainController?action=booking" class="btn btn-primary-gradient px-4 py-2 fw-bold">
                        ...
                        <c:when test="${not empty historyList}">
                            <c:forEach var="app" items="${historyList}">
                                <tr>
                                    ...
                                    <td>
                                        <div class="fw-bold text-info"><i class="fa-solid fa-calendar-day me-1"></i>${app.appointmentDate}</div>
                                        <div class="small text-muted"><i class="fa-solid fa-clock me-1"></i>${app.startTime}</div>
                                    </td>
                                    <td class="text-white">
                                        <i class="fa-solid fa-user-doctor me-1 text-info"></i>${app.doctorName}
                                    </td>
                                    <td>
                                        <span class="badge px-3 py-2 rounded-pill fw-bold" style="background: rgba(13, 202, 240, 0.15); color: #0dcaf0 !important; border: 1px solid rgba(13, 202, 240, 0.4);">
                                            <i class="fa-solid fa-notes-medical me-1"></i>${app.serviceName}
                                        </span>
                                    </td>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${app.paymentStatus == 'PAID'}">
                                                <span class="badge bg-success bg-opacity-20 border border-success border-opacity-25 px-3 py-2 rounded-pill fw-bold" style="color: #00ff88 !important;">
                                                    <i class="fa-solid fa-circle-check me-1"></i>Đã Thanh Toán<c:if test="${not empty app.paymentMethod}"> (${app.paymentMethod})</c:if>
                                                </span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="badge bg-warning bg-opacity-20 border border-warning border-opacity-25 px-3 py-2 rounded-pill fw-bold" style="color: #ffc107 !important;">
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${app.status == 'COMPLETED'}">
                                                <span class="badge bg-success"><i class="fa-solid fa-circle-check me-1"></i>Hoàn Tất Khám</span>
                                            </c:when>
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex gap-1 justify-content-end">
                                            <%-- Nút Thanh Toán VietQR SePay nếu chưa trả tiền --%>
                                            <c:if test="${app.paymentStatus == 'UNPAID' && app.status != 'CANCELLED'}">
                                                <a href="${pageContext.request.contextPath}/booking?action=payment&id=${app.id}" class="btn btn-sm btn-success fw-bold">
                                                    <i class="fa-solid fa-qrcode me-1"></i>Thanh Toán QR
                                                </a>
                                            </c:if>

                                            <%-- Nút Xem Đơn Thuốc & Chẩn Đoán nếu ca khám đã hoàn tất --%>
                                            <c:if test="${app.status == 'COMPLETED' && not empty recordsMap[app.id]}">
                                                <button type="button" class="btn btn-sm btn-info fw-bold"
                                                    ...
                                                    <i class="fa-solid fa-file-medical me-1"></i>Xem Đơn Thuốc
                                                </button>
                                            </c:if>
                                        </div>
                                    </td>
                        ...
                        <c:otherwise>
                            <tr>
                                <td colspan="8" class="text-center py-5 text-muted">
                                    ...
                                    <a href="${pageContext.request.contextPath}/MainController?action=booking" class="text-info fw-bold ms-1">Đặt lịch ngay</a>
                                </td>
                            </tr>
                        </c:otherwise>
                    </c:choose>
        ...
        <nav aria-label="Page navigation" class="animate-fade-in">
            <ul class="pagination justify-content-center">
                ...
                <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                    <a class="page-link bg-dark text-white border-secondary" href="${pageContext.request.contextPath}/history?page=${currentPage - 1}" aria-label="Previous">
                ...
                <c:forEach var="i" begin="1" end="${totalPages}">
                    <li class="page-item ${currentPage == i ? 'active' : ''}">
                        <a class="page-link ${currentPage == i ? 'btn-primary-gradient text-white border-0' : 'bg-dark text-info border-secondary'}" href="${pageContext.request.contextPath}/history?page=${i}">
                            ${i}
                        </a>
                    </li>
                </c:forEach>
                ...
                <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                    <a class="page-link bg-dark text-white border-secondary" href="${pageContext.request.contextPath}/history?page=${currentPage + 1}" aria-label="Next">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content glass-card border border-info border-opacity-30 text-white">
            ...
            <div class="modal-body">
                ...
                <div class="mb-4">
                    <h6 class="fw-bold text-info mb-2"><i class="fa-solid fa-stethoscope me-2"></i>Chẩn Đoán Bệnh Lý:</h6>
